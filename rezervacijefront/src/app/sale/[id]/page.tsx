...
export default function SalaDetalji({ params }: PageProps) {
  ...
  const [selectedKarakteristike, setSelectedKarakteristike] = useState<
    number[]
  >([]);
  ...
  useEffect(() => {
    api
      ...
      .then((data) => {
        setSala(data);
        setSelectedKarakteristike(
          data.karakteristike?.map((k: any) => k.id) || [],
        );
    ...
    //novo
    fetch("http://localhost:8080/api/karakteristike")
      .then((res) => res.json())
      .then(setSveKarakteristike)
      .catch((err) => console.error("Greška karakteristike:", err));
    fetch("http://localhost:8080/api/tipovidogadjaja")
      .then((res) => res.json())
      .then(setSviTipovi)
      .catch((err) => console.error("Greška tipovi:", err));
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          "Greška pri čuvanju: " + (errorData.message || "Proverite podatke."),
        );
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white border-b px-8 py-4 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <Link
            href="/"
            className="text-pink-600 font-bold flex items-center gap-2 hover:translate-x-[-4px] transition-transform"
          >
            ← NAZAD NA SVE SALE
          </Link>
              ...
              <img
                src={
                  !sala.slike
                    ...
                    : sala.slike.startsWith("http")
                      ? sala.slike // Ako je pun URL iz baze, koristi ga direktno
                      : sala.slike.startsWith("slike/")
                        ? `http://localhost:8080/storage/${sala.slike}` // putanja za slike u storage folderu
                        : `/slike/${sala.slike.split(",")[0].trim()}` // za lokalne fajlove
                }
                alt={sala.naziv}
              ...
              {!isAdmin ? (
                <button
                  ...
                  onClick={() => {
                    if (!currentUser) {
                      alert(
                        "Morate biti ulogovani da biste rezervisali termin!",
                      );
                ...
                <div className="mt-8 p-4 bg-blue-50 border border-blue-100 rounded-2xl text-blue-700 text-sm font-medium text-center">
                  ...
                  <span className="text-xs opacity-75 italic">
                    Rezervacije su onemogućene za ovaj nalog.
                  </span>
                </div>
              )}
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {sala.tipovi_dogadjaja && sala.tipovi_dogadjaja.length > 0 ? (
                  sala.tipovi_dogadjaja.map((t) => (
                    <span
                      key={t.id}
                      className="bg-pink-50 text-pink-700 px-3 py-1.5 rounded-full text-sm font-medium"
                    >
                      {t.naziv}
                    </span>
                  ))
                  ...
                  <span className="text-gray-400 text-xs italic">
                    Nema definisanih tipova događaja
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-[40px] w-full max-w-2xl max-h-[90vh] overflow-y-auto p-10 shadow-2xl relative">
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-500 font-medium">
                Sve izmene će biti odmah vidljive na sajtu.
              </p>
            </div>
